import React from 'react'
import Slider from 'react-slick'

const Testimonaildata= [
    {
        id: 1,
        name: "John Doe",
        image: "https://pisum.photos/101/101",
        review: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatibus.", 
    },    
    {
        id: 2,
        name: "Jane Smith",
        image: "https://pisum.photos/102/102",
        review: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatibus.",                 
    },
    {
        id: 3,
        name: "Alice Johnson",
        image: "https://pisum.photo/103/103",
        review: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatibus.",     
    },          
    {
        id: 4,
        name: "Bob Brown",
        image: "https://pisum.photos/104/104",
        review: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatibus.", 
    },
    {
        id: 5,
        name: "Charlie Davis",
        image: "https://pisum.photos/105/105",
        review: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatibus.",     
    },
   
];
const Testimonials = () => {
    const settings = {
        dots: true,
        infinite: true,
        speed: 500,
        slidesToShow: 3,
        slidesToScroll: 1,
        autoplay: true,
        autoplaySpeed: 2000,
        cssEase: "linear",
        arrows: false,
        pauseOnHover: true,
        pauseOnFocus: true,
        responsive: [
            {
                breakpoint: 1024,
                settings: {
                    slidesToShow: 3,
                    slidesToScroll: 1,
                    infinite: true,
                    dots: true,
                },
            },
            {
                breakpoint: 1024,
                settings: {
                    slidesToShow: 2,
                    slidesToScroll: 1,
                    initialSlide: 2,
                },
            },
            {
                breakpoint: 640,
                settings: {
                    slidesToShow: 1,
                    slidesToScroll: 1,
                },
            },
        ],  
        
      };
  return (
    <div className='py-10 mb-10'>
        <div className='container'>
                {/* header section  */}
                <div className='text-center 
                mb-10 max-w-[600px] mx-auto'>
                <p data-aos="fade-up" 
                className='text-sm text-primary'>
                    What your customers are Saying</p>
                <h1 data-aos="fade-up"  className='text-3xl font-bold'>
                    Testimonials</h1>
                <p data-aos="fade-up" className='text-xs
                 text-gray-400'>
                    Lorem ipsum dolor sit amet consectetur adipisicing 
                    elit. Perferendis rem magnam pariatur
                     exercitationem enim sequi,
                      recusandae amet et sit commodi.</p>
            </div>
            {/* testimonails  */}
            <div
            data-aos="zoom.in"
            >
                <Slider {...settings}>
                    { Testimonaildata.map((data) => (
                        <div className='my-6'>
                     <div
                        key={data.id}
                        className='flex flex-col rounded-xl shadow-lg gap-4 
                        py-8 px-6 mx-4 Dark:bg-gray-800 bg-primary/10 relative'
                        > 
                                    <div className='mb-4'>
                                    <img src={data.image} alt="" 
                                    className='rounded-full 
                                    w-20 h-20
                                    '
                                    />
                                </div>
                                <div className='flex flex-col items-center gap-4'>
                                    <div
                                    className='space-y-3'>
                                    <p 
                                    className='text-xs text-gray-400'>{data.text}</p>
                                    <h1 className='text-xl font-bold
                                    text-black/80 Dark:text-light '>{data.name}</h1>
                                </div>
                              </div>  
                              <p className='text-black/20 text-9xl
                              font-serif absolute top-0 right-0'></p>
                        </div>
                        </div>
                    ))}
                </Slider>
                
            </div>
        </div>
    </div>
  )
}

export default Testimonials